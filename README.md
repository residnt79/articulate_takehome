...
- **Monthly Recurring Revenue (MRR)** - Sum of monthly plan prices for active subscriptions in the month.
...
I recalculated subscription_status by:
  - Creating a customer-month spine based on each customers signup_date.
  - Identifying subscriptions that overlap each month. 
...
``` 
**Assumptions:**
- A subscription is "Active" for a month if it overlaps any portion of that month.
- Subscriptions are assumped to span at least one day. Start date and End date are not equal.
...
- **Completeness:** Representing every customer in every month after signup makes lifecycle transitions explicit and simplifies churn analysis using window functions.
...
- Churn is recorded in the first month where a customer has no active subscription.
